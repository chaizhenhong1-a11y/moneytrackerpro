...
import '../../domain/services/financial_action_plan_service.dart';
import '../../domain/services/safe_to_spend_service.dart';
import '../../domain/services/spending_pace_forecast_service.dart';
import 'budget_stress_test_page.dart';
import 'emergency_fund_planner_page.dart';
import 'financial_action_plan_page.dart';
import 'income_stability_planner_page.dart';
import 'safe_to_spend_page.dart';
import 'savings_target_planner_page.dart';
import 'spending_guard_page.dart';
import 'spending_pace_page.dart';
import 'surplus_allocation_page.dart';

class MonthlyFinancialReviewPage extends StatelessWidget {
  const MonthlyFinancialReviewPage({required this.review, super.key});
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: () {
              final plan = FinancialActionPlanService.build(review);
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => FinancialActionPlanPage(plan: plan),
                ),
              );
            },
            icon: const Icon(Icons.task_alt_rounded),
            label: const Text('Build my action plan'),
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            onPressed: () {
              final plan = SafeToSpendService.build(review);
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => SafeToSpendPage(plan: plan),
                ),
              );
            },
            icon: const Icon(Icons.account_balance_wallet_outlined),
            label: const Text('See my safe-to-spend limit'),
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => SpendingGuardPage(review: review),
                ),
              );
            },
            icon: const Icon(Icons.shield_outlined),
            label: const Text('Check a purchase with Spending Guard'),
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            onPressed: () {
              final forecast = SpendingPaceForecastService.build(review);
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => SpendingPacePage(forecast: forecast),
                ),
              );
            },
            icon: const Icon(Icons.speed_rounded),
            label: const Text('View spending pace & month-end projection'),
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => BudgetStressTestPage(review: review),
                ),
              );
            },
            icon: const Icon(Icons.science_outlined),
            label: const Text('Run a budget stress test'),
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => EmergencyFundPlannerPage(review: review),
                ),
              );
            },
            icon: const Icon(Icons.health_and_safety_outlined),
            label: const Text('Plan my emergency fund'),
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => SurplusAllocationPage(review: review),
                ),
              );
            },
            icon: const Icon(Icons.pie_chart_outline_rounded),
            label: const Text('Allocate this month\'s surplus'),
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => SavingsTargetPlannerPage(review: review),
                ),
              );
            },
            icon: const Icon(Icons.savings_outlined),
            label: const Text('Set a savings-rate target'),
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => IncomeStabilityPlannerPage(review: review),
                ),
              );
            },
            icon: const Icon(Icons.trending_up_rounded),
            label: const Text('Check my income stability'),
          ),
        ],
      ),
    );
